...
@Injectable()
export class GetEvaluationChallengeSubmissionsUseCase {
  constructor(
  ...
  async execute(studentId: string, evaluationId: string, challengeId: string): Promise<SubmissionResponseDto[]> {
    ...
    const submissions = await this.submissionRepository.findByStudentAndChallenge(studentId, challengeId);
